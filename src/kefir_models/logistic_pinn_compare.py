...
def transition_negative_log_likelihood(
    ...
    drift = (
        parameters.growth_rate
        * current
        * (1.0 - current / parameters.carrying_capacity)
    )
    ...
    return 0.5 * (
        torch.log(2.0 * math.pi * variance) + squared_error / variance
    ).mean()
...
def simulate_logistic_sde(
    ...
    for index in range(len(time_tensor) - 1):
        ...
        current = (
            current
            + drift * delta_time
            + diffusion_term * torch.sqrt(delta_time) * noise
        ).clamp_min(1e-6)
...
def build_metrics_frame(
    ...
    sde_mean = comparison_frame[
        [f"{column}_sde_mean" for column in data.trial_columns]
    ].to_numpy()
...
def save_outputs(
    ...
    deterministic_history_path = (
        output_dir / "deterministic_logistic_pinn_training_loss.csv"
    )
