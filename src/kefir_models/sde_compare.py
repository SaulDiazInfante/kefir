...
def train_classical_logistic_model(
    ...
    for epoch in range(1, epochs + 1):
        ...
        carrying_capacity = (
            minimum_capacity + F.softplus(raw_capacity_margin) + 1e-6
        )
    ...
    with torch.no_grad():
        for index in range(len(time_points) - 1):
            ...
            current = (
                ...
                + model.diffusion(time, current)
                * torch.sqrt(delta_time.clamp_min(1e-8))
                * noise
            )
...
def gaussian_information_criteria(
    ...
    bic = (
        math.log(observation_count) * likelihood_parameter_count
        - 2.0 * log_likelihood
    )
